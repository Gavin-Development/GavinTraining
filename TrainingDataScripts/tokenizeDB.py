...
def process_databases(t_databases, thread_id):
    for database in t_databases:
        print(f"Thread: {thread_id}: Starting Work on: {database}")
        limit = 3_000_000
        ...
        while cur_length == limit:
            try:
                ...
                for parent, comment in list(zip(df['parent'].values, df['comment'].values)):
                    ...
                    cursor.execute(sql)
                    i += 1
        print(f"Thread: {thread_id}: Starting Vacuum on: {database}")
        connection.commit()
        cursor.execute("VACUUM")
        connection.commit()
        print(f"Thread: {thread_id}: Finished Vacuum on: {database}")
        connection.close()
        shutil.move('./temp/{}'.format(database), 'D:/Datasets/reddit_data/databases/{}'.format(database))
        print(f"Thread: {thread_id}: Finished Work on: {database}")
...
if __name__ == '__main__':
    ...
    generator = chunk(databases, cores)
    with ThreadPoolExecutor(cores) as executor:
        for i in range(cores):
            executor.submit(process_databases, next(generator), i)
