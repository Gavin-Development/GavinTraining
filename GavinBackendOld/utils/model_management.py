...
def model_load(log_dir: str, dataset_path: str) -> Tuple[tf.keras.models.Model, tf.data.Dataset, tf.data.Dataset]:
    hparams = json.load(fp=open(os.path.join(log_dir, os.path.join("values/", "config.json"))))
...
def create_model(log_dir: str, dataset_path: str, hparams: Union[dict, OrderedDict], load: bool = False) -> Tuple[tf.keras.models.Model, tf.data.Dataset, tf.data.Dataset]:
    ...
    loaded_tokenizer = tfds.deprecated.text.SubwordTextEncoder.load_from_file(
        f"{log_dir}/tokenizer/vocabTokenizer")
